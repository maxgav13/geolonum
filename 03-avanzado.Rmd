...
Operadores lógicos permiten hacer comparaciones o pruebas, donde usualmente el resultado es `TRUE` o `FALSE`. En términos numéricos `TRUE` equivale a 1 y `FALSE` a 0. Los operadores lógicos mas usados son:
...
Aquí se crea un vector numérico, y se tratan de extraer los valores menores a 70. Se muestra la forma básica de **R** (`vector[condicion]`) y una forma mas directa e intuitiva que ofrece *purrr*.
...
Dos de las funciones auxiliares mas útiles son:

* `between(x,left,right)` que filtra observaciones para la variable `x` que se encuentren entre `left` (limite inferior) y `right` (limite superior); esta es mas útil para variables numéricas,
* `x %in% c(a,b,c)` que filtra observaciones para la variable `x` que se encuentren en el vector `c(a,b,c)`; esta es mas útil para variables de texto o factor
...
En el tercer ejemplo se re define la variable 'Month' pasándola a factor donde se le cambian las etiquetas a algo mas explicito. A su vez, se define una nueva variable condicionada en los valores de otra (sensación dependiendo del valor de la temperatura). Aquí se ejemplifica `case_when`, donde la estructura es:
...
Para contar casos de variables discretas de una manera mas expedita se puede usar `count`. Esta función realiza un agrupamiento (`group_by`) y resumen (`summarise`) a la vez.
