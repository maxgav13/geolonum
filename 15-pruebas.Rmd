...
* Prueba $t$ de 2 muestras dependientes (\@ref(prueba-t2-dep))
  - $H_0: \mu_D = 0$
* Prueba ANOVA 1-factor entre-sujetos (\@ref(prueba-aov))
  - $H_0: \mu_1 = \mu_2 = \mu_3 = \cdots = \mu_k$
* Correlación de Pearson (\@ref(prueba-pearson))
  - $H_0: \rho = 0$
* Correlación de Pearson (\@ref(prueba-rpb))
  - $H_0: \rho_{pb} = 0$
...
La corrección de Hedges ($g$) se usa ya que $d$ se considera un estimador sesgado (biased) especialmente para muestras pequeñas, conforme mayor sea el tamaño de muestra más similares $d$ y $g$ [@cumming2012; @cumming2017; @fritz2012joepg; @mcgrath2006pm]. De manera similar $\omega^2$ corresponde con un estimador insesgado o menos sesgado de $\eta^2$ [@fritz2012joepg; @lakens2013fp; @olejnik2000cep; @tomczak2014tss].

Así como se puede calcular el tamaño del efecto a partir de los datos, en algunos casos se puede calcular el tamaño del efecto a partir de los resultados de pruebas estadísticas u otros tamaños de efecto. Esta relación se muestra en la Tabla \@ref(tab:ES-conv). En su mayoría estas relaciones son de utilidad cuando se revisan o quieren comparar estudios que reportan únicamente resultados de pruebas estadísticas pero no así el tamaño del efecto.

```{r ES-conv, echo=FALSE}
ESconv = tibble(
  Prueba = c('t de 1 muestra',
             't de 2 muestras independientes',
             't de 2 muestras dependientes usando la desviación de las diferencias',
             't de 2 muestras dependientes tomando en cuenta la correlación',
             'Correlación punto biserial',
...
ESconv %>% 
  kable(col.names = c('Prueba',
        ...
        caption = 'Tamaños de efecto a partir de pruebas estadísticas u otros tamaños de efecto',
        align = 'c',
        escape = F) %>% 
  kable_styling(full_width = T) %>% 
  column_spec(1,width = '15em') %>% 
  column_spec(2,width = '25em') %>% 
  footnote(general_title = 'Notas:',
           general = c('$t$ = estadístico de la prueba respectiva',
                       '$n_1$ = tamaño de muestra 1',
                       '$n_2$ = tamaño de muestra 2',
                       '$N$ = total de observaciones',
                       '$r$ = coeficiente de correlación de Pearson',
...
### Clasificación de tamaños de efecto
...
ESpc = tibble(Prueba = c('Comparación de medias',
              ...
              ES = c('$d$, $g$',
                     ...
                     '$R^2$',
                     '$\\eta^2$, $\\omega^2$'),
              peque = c(apa(0.20,2,T), apa(0.10,2,F), apa(0.01,2,F), apa(0.1,2,F), apa(0.02,2,F), apa(0.01,2,F)),
              mediana = c(apa(0.50,2,T), apa(0.30,2,F), apa(0.09,2,F), apa(0.24,2,F), apa(0.13,2,F), apa(0.06,2,F)),
              grande = c(apa(0.80,2,T), apa(0.50,2,F), apa(0.25,2,F), apa(0.37,2,F), apa(0.26,2,F), apa(0.14,2,F))
              )
...
A primera vista se podría concluir que los resultados de ambos estudios no coinciden y encontraron efectos diferentes. ¿Qué pasa con el tamaño del efecto? Si calculamos $d$ a aprtir de la Ecuación \@ref(eq:d-t2-t) para ambos estudios tenemos lo siguiente:
...
El uso de la prueba $t$ de 1 muestra se realiza con el ejemplo de la sección \@ref(estim-ic-t) [@swan1995], donde se tenía el contenido de cuarzo en secciones delgadas de una roca ígnea. Es posible que esta muestra provenga de una población con media 20% ($\mu_0=20$)? Asuma $\alpha = .05$.
...
```{r echo=FALSE}
t2.ind.stats = TeX(str_interp("$\\mathit{t}(${t2.ind.res$parameter}) = ${apa(t2.ind.res$statistic,2)}, \\mathit{p} ${printp(t2.ind.res$p.value,add_equals = T)}, \\mathit{d_s} = ${apa(d.t2.ind.res[[1]],2)}, IC_{${(1-a)*100}%} \\[${apa(d.t2.ind.res[[2]],2)}, ${apa(d.t2.ind.res[[3]],2)}\\]"))
```
...
El uso de la prueba $t$ de 2 muestras dependientes se realiza con el ejemplo de la sección \@ref(estim-ic-t2-dep) [@mckillup2010], donde se tenía el contenido de $FeO$ en porcentaje de peso para 10 granitos que fueron preparados a diferentes tamaños de grano: $< 25 \ \mu m$ y $< 125 \ \mu m$. Hay un efecto en el tratamiento, en este caso, cambio del tamaño de grano ($\mu_D=0$)? Asuma $\alpha = .05$.
...
## ANOVA de 1-factor entre-sujetos {#prueba-aov}
...
Para demostrar el ANOVA se usa un ejemplo de @mckillup2010, donde se tiene el contenido de $MgO$ presente en cuatro turmalinas en tres sitios diferentes: Mount Mica, Sebago Batholith, Black Mountain. Asuma $\alpha=.05$.
...
El tamaño del efecto $\eta^2$ se puede calcular usando las Ecuaciones \@ref(eq:eta2-1), \@ref(eq:eta2-2), y el tamaño del efecto $\omega^2$ se puede calcular usando las Ecuaciones \@ref(eq:omega2-1), \@ref(eq:omega2-2), donde $SC_{total}$ es la suma de la columna *Suma de cuadrados (Sum Sq)*. Para este tipo de ANOVA (1-factor entre grupos) se cumple que $\eta^2 = \eta_p^2$ y $\omega^2 = \omega_p^2$, ya que hay solo 1 factor, pero para el resto de ocasiones donde haya más de un factor esta igualdad no se cumple y se recomienda incluir ambos tamaños de efecto (global y parcial).
...
```{r}
aov.eta.p = eta.partial.SS(dfm = 2, dfe = 9, 
                           ssm = 72, sse = 30,
                           Fvalue = 10.8, a = a)
aov.eta.p[1:3] %>% unlist()
```

```{r}
eta_squared(mgo.aov,partial = T,ci = 1-a)
```
...
aov.omega[1:3] %>% unlist()
```

```{r}
aov.omega.p = omega.partial.SS.bn(dfm = 2, dfe = 9, 
                                  msm = 36,mse = 3.33,
                                  ssm = 72, n = N, a = a)
aov.omega.p[1:3] %>% unlist()
```

```{r}
omega_squared(mgo.aov,partial = T,ci = 1-a)
```
...
### Relación entre la prueba $t$ de 2 muestras independientes y ANOVA de 1-factor entre-sujetos
...
El resultado de la prueba $t$ de 2 muestras independientes tenía $t = `r apa(t2.ind.res[['statistic']],2)`$, y $p = `r apa(t2.ind.res[['p.value']],3,F)`$. Si comparamos estos valores con el resultado de ANOVA se tiene que el valor-*p* es el mismo, y $t^2=F \to 1.86^2=3.46$, que efectivamente es el valor de $F$.

Lo otro que se puede comparar son los tamaños de efecto. Para la prueba $t$ de 2 muestras independientes se tenía $d_s = `r apa(d.t2.ind.res[[1]],3)`$, y para el ANOVA de los mismos datos se tiene $\eta_p^2 = `r apa(EtaSq(braq.aov)[[2]],3,F)`$ (el lector puede hacer el cálculo para corroborar). 

Para pasar de $d_s$ a $\eta_p^2$ se pueden usar las Ecuaciones \@ref(eq:r-pb-d) para obtener $r_{pb}$ y $\eta_p^2 = r_{pb}^2$, o \@ref(eq:r-pb-eta2) para obtener $\eta_p^2$ directamente, como se demuestra a continuación. 
...
## Correlación de Pearson {#prueba-pearson}
...
Para determinar la significancia de la correlación de Pearson se usa una prueba $t$. Esta prueba se realiza con un ejemplo de @davis2002, donde se tiene la longitud de los ejes de cantos en una playa (Tabla \@ref(tab:cantos-playa)). Se quiere determinar si la correlación entre los ejes $a$ y $b$ es significativa, asumiendo $\alpha = .1$.
...
```{r echo=FALSE}
rpb.apa = apa_print(rpb.res)
```

Estos resultados muestran el mismo valor $t$ y mismo valor-*p* que la prueba $t$ de 2 muestras independientes, demostrando que ambas son homólogas.

> Conclusión: La longitud de los braquiópos no está correlacionada con la capa a la cual pertenecen, `r rpb.apa$full_result %>% str_replace_all(c('CI'='IC','r'='r_{pb}'))`. El efecto se puede considerar mediano, con un rango amplio de muy pequeño, en la dirección opuesta, hasta muy grande.

```{r rpb-stats, echo=FALSE, out.width='90%', fig.cap='Gráfico de dispersión mostrando la relación entre longitud de los braquiópodos de acuerdo a la capa que pertencen, así como el resumen estadístico respectivo.'}
set.seed(101)
braq %>% 
  ...
  labs(x = 'Capa',
       y = 'Longitud (cm)',
       subtitle = TeX(str_interp("$\\mathit{r_{pb}} = ${apa(rpb.res$estimate,2,F)}, IC_{${(1-a)*100}%} \\[${apa(rpb.res$conf.int[[1]],2,F)}, ${apa(rpb.res$conf.int[[2]],2,F)}\\], \\mathit{p} ${printp(rpb.res$p.value,add_equals = T)}"))) + 
...
El uso de la prueba $\chi^2$ de 1 muestra se realiza con el ejemplo de la sección \@ref(estim-ic-chi) [@swan1995], donde se tenía el contenido de cuarzo en secciones delgadas de una roca ígnea. Es posible que esta muestra provenga de una población con varianza 12 ($\sigma^2_0=12$)? Asuma $\alpha = .05$.
...
El uso de la prueba $F$ para 2 varianzas se realiza con el ejemplo de la sección \@ref(prueba-t2-ind) [@swan1995], donde se tenían braquiópodos en dos capas (A, B) y se les midió la longitud (cm). Es factible la suposición de igualdad de varianzas ($\sigma^2_1=\sigma^2_2$)? Asuma $\alpha = .05$.
