...
El paquete *tmap* es una opcion especifica para datos espaciales y sigue la misma ideologia de *ggplot* al trabajar en capas. Por defecto no despliega la grilla de coordenadas, hay que agregarselas con `tm_grid`. Para revertir los colores en la paleta se le agrega un menos (`-`) antes del nombre, y para colocar la leyenda fuera del area del mapa se debe usar `tm_layout(legend.outside = T)`.
...
Una de la opciones con mapas interactivos es que se pueden definir diferentes mapas de fondo. Dentro de los mas usados estan los que se muestran acontinuacion, y de manera general se pueden asignar para todos los mapas de una sesion definiendolos en las opciones de *mapview*:
