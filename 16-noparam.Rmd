...
* Correlación de Spearman (\@ref(prueba-spearman))
  - $H_0: r_s = 0$ No hay relación/asociación entre las variables ordinales
...
Para mostrar la prueba de bondad de ajuste se usa el ejemplo de @mckillup2010, donde se tiene una muestra de 20 foraminíferos, clasificados en enrollados a la derecha (16) o a la izquierda (4), y se quiere comparar con las proporciones de 0.9 y 0.1 respectivamente. Asuma $\alpha=0.05$. 
...
4. Determinar valores críticos
    - $\alpha = 0.05$
    - $\chi^2_{\alpha,v} = \chi_{0.05,1}^2=3.84$
5. Calcular el estadístico de prueba
    - Para un $N=20$ la proporción de 0.9 equivale a 18 y la proporción de 0.1 equivale a 2, que serían las frecuencias esperadas (*Con estos valores, especialmente el de 2, se estaría violando el supuesto de tener 5 o más frecuencias esperadas*)
    - $\chi^2 = \sum_{K=1}^K\frac{(O_K-E_K)^2}{E_K}$
...
6. Tomar una decisión
    ...
    - El valor-*p* es mayor a $\alpha = 0.05$, $p = 0.136$
...
\begin{equation}
  \phi = \sqrt{\frac{\chi^2}{N}} = \sqrt{\frac{2.22}{20}} = 0.33
\end{equation}
...
En caso de que se encuentre un resultado significativo (rechazar $H_0$), ya sea para la prueba de homogeneidad o de independencia, es importante explicar dónde o cuáles celdas (combinaciones de categorías) de la tabla de contingencia se desvían de los valores esperados. Para ésto se pueden analizar los residuales ajustados (un tipo de análisis posterior - post-hoc), con lo que cuando un residual ajustado es superior a $|1.96|$, para un $\alpha=0.05$, esta celda se considera se desvía significativamente de la hipótesis nula, mostrando dónde se dan estas diferencias entre los valores observados y esperados, pudiendo ayudar a explicar la relación entre las variables.

#### Homogeneidad {#prueba-chi-hom}

Para ejemplificar la prueba de homogeneidad se usa el ejemplo de @mckillup2010 (Tabla \@ref(tab:pozos)), donde se tienen 20 mediciones de pozos en 3 localidades diferentes, a los cuales se les determinó si se encontraban contaminados o no con nitratos. Asuma $\alpha=0.05$. 
...
4. Determinar valores críticos
    - $\alpha = 0.05$
    - $\chi^2_{\alpha,v} = \chi_{0.05,2}^2=5.99$
5. Calcular el estadístico de prueba
...
6. Tomar una decisión
    - El estadístico de prueba es menor al crítico, $\chi^2 < \chi_{\alpha,v}^2$
    - El valor-*p* es mayor a $\alpha = 0.05$, $p = 0.072$
...
Los residuales ajustados se pueden acceder a partir del objeto en que se guarda la prueba, como se muestra en el siguiente bloque. Estos residuales son los que realmente indican si hay una diferencia significativa entre lo observado y lo esperado ($> |1.96|$ para $\alpha=0.05$), que para el caso del ejemplo ocurre en la localidad de Bowen, donde hay menos valores observados para lo contaminado y más valores observados para lo no contaminado.

```{r}
chi.h$stdres
```

Estos valores se pueden visualizar mejor coloreados de acuerdo a si están por encima o debajo del valor crítico de acuerdo al nivel de significancia $\alpha$.
...
\begin{equation}
  V = \sqrt{\frac{\chi^2}{N \cdot v_{min}}}\\
  V = \sqrt{\frac{5.25}{60 \cdot 1}} = 0.29
\end{equation}
...
Para ejemplificar la prueba de homogeneidad se usa el ejemplo de @borradaile2003 (Tabla \@ref(tab:dist-red)), donde se tienen 246 mediciones de clastos, los cuales se clasificaron de acuerdo a la redondez y la distancia de la fuente. Asuma $\alpha=0.05$. 
...
4. Determinar valores críticos
    - $\alpha = 0.05$
    - $\chi^2_{\alpha,v} = \chi_{0.05,12}^2=21.03$
5. Calcular el estadístico de prueba
...
\begin{equation}
  V = \sqrt{\frac{\chi^2}{N \cdot v_{min}}}\\
  V = \sqrt{\frac{109.7}{264 \cdot 3}} = 0.38
\end{equation}
...
El uso de la prueba se demuestra con el ejemplo @swan1995, donde se tenía el contenido de cuarzo en secciones delgadas de una roca ígnea. Es posible que esta muestra provenga de una población con valor central de 20%? Asuma $\alpha = 0.05$.
...
\begin{equation}
  r = \frac{Z}{\sqrt{N}} = \frac{-1.33}{\sqrt{8}} = -0.47
\end{equation}
...
El uso de la prueba se demuestra con el ejemplo @swan1995, donde se tenían braquiópodos en dos capas (A, B) y se les midió la longitud (cm). Es posible que estas muestras provenga de una misma población? Asuma $\alpha = 0.05$.
...
\begin{equation}
  r = \frac{Z}{\sqrt{N}} = \frac{-1.46}{\sqrt{18}} = -0.35
\end{equation}
...
El uso de la prueba se demuestra con el ejemplo de @mckillup2010, donde se tiene el contenido de $MgO$ presente en cuatro turmalinas en tres sitios diferentes ($k=3$): Mount Mica, Sebago Batholith, Black Mountain. Asuma $\alpha = 0.05$.

El resultado de realizar el ranqueo y la suma de los respectivos rangos de acuerdo a las muestras se detalla en las Tablas \@ref(tab:kw1) y \@ref(tab:kw2). El estadístico crítico sería $\chi^2_{\alpha,v} = \chi^2_{0.05,2} = 5.99$.
...
\begin{equation}
  \eta_H^2 = \frac{H-k+1}{N-k} = \frac{7.54-3+1}{12-3} = 0.62
\end{equation}

\begin{equation}
  \epsilon^2 = \frac{H}{(N^2-1)/(N+1)} = \frac{7.54}{(12^2-1)/(12+1)} = 0.69
\end{equation}
...
- $H_0: r_s = 0 \to$ No hay relación entre las variables,
...
El uso de la prueba de correlación de Spearman se realiza con un ejemplo de @swan1995, donde se el tamaño de ostrácodos con respecto a la profundidad (Tabla \@ref(tab:spearman-ej1)). Hay una relación entre el paso del tiempo (mayor profundidad) y el tamaño de los ostrácodos? Asuma $\alpha = 0.05$.
...
\begin{equation}
  r_{s} = 1 - \frac{6\sum_i^N[R(x_i)-R(y_i)]^2}{N(N^2-1)}\\
  r_{s} = 1 - \frac{6 \cdot 1941.5}{33(33^2-1)} = 0.675
\end{equation}
...
Al ser $t$ mayor al valor crítico, $t_{0.05/2,31}=|2.04|$, y al ser $z$ mayor al valor crítico, $1.96$ para $\alpha=0.05$, se obtiene la misma decisión que con la prueba respectiva.
