...
ggplot(airquality,aes(x=Temp)) +
  geom_histogram(bins = 20,aes(fill=stat(count)),col="black") +
  scale_fill_distiller(palette = 'YlOrRd')
```

Asi como se puede modificar el relleno de las barras, se puede modificar que el eje y no corresponda con el conteo sino con la densidad (Figura \@ref(fig:graficos-hist6)), para esto es necesario insertar dentro del `aes()` de `geom_histogram` el `y=stat(density)`, y esto es necesario si se quiere agregar la curva de densidad de los datos (`geom_density`) para ver su distribucion.
